...
def save_net(netD, netG):
    torch.save(netD, 'saved_models/task2/best_model_netD.pkl')
    torch.save(netG, 'saved_models/task2/best_model_netG.pkl')

def load_net():
    netD = torch.load('saved_models/task2/best_model_netD.pkl')
    netG = torch.load('saved_models/task2/best_model_netG.pkl')
    netD.eval()
    netG.eval()
    return netD, netG
...
def main(dataset_path, train_epochs, loss,loss1, optimizer, batch_size, learning_rate, device,train_size, Training=False, Visualize=False):   
    ...
    if Visualize ==True:
        ...
        netD, netG = load_net() 
        history = np.load('saved_models/task2/history.npy',allow_pickle=True)
...
if __name__ == '__main__':
    ...
    main(dataset_path, train_epochs, loss,loss1, optimizer, batch_size, learning_rate, device, train_size,Training=False, Visualize=True)